...
> 基准日 **2026-08-30** ｜ 构建镜像 **9** 个（3 发行版 × 3 档位）｜ 构建路径 **3** 条 ｜ 能力矩阵 **648** 格 ｜ 验收断言 **365** 条（镜像层，`make verify`）｜ 机器核对断言 **471** 条（文档层，`scripts/verify.py`） ｜ 变异用例 **12**（镜像层）+ **15**（分析层）条 ｜ 厂商缺陷留档 **12** 条 ｜ 一手数据集 **8** 组 ｜ 图 **6** 张 ｜ 可复算表 **18** 张 ｜ 参考来源 **125** 条
...
对名录里除三个被试之外的 18 个 OS 逐条套三条排除标准，再过一道「交付端是否真有验证需求」。
...
| ② 官方已有能满足对标需求的交付形态 | openEuler、Anolis OS、OpenCloudOS | 三家**本来就没有独立桌面 ISO**，桌面是「官方基础镜像 + 按包装」（Anolis 有独立 DDE 仓库、OpenCloudOS 文档有 GNOME 43 装法）。从 ISO 切属于做错方向：那条路更长，还要自己承担「装出来的与真机一致吗」的举证责任 |
| ③ ISO 拿不到 | 方德、EulerOS、新支点 | 方德桌面版查遍官网栏目与两台公开更新服务器（900 + 119 个目录）无条目；EulerOS 门户里根本没有 ISO 条目（正对照见 §2.1）；新支点桌面版**仅经百度网盘与微云分发**——拿得到但不可脚本化，可复现链会断在人工下载那一步；另外它的桌面线活跃度本身也存疑，近两年公开落地与中标全在服务器/嵌入式侧，桌面线自身的最近更新时间未查到 |
...
| **凝思安全操作系统** | **场景类型**：调度席位而非办公终端 | 血统证据是全名录最硬的一条（`grub2 2.06-3~deb10u4linx5` + `iceweasel`），deb 系可直接套 §4.1／§4.2。取材也最省事：`download/` 是开放 autoindex，22 个 ISO 匿名可下，如 `linxos-v6.0.99-el20.03-20260605-x86_64.iso` 7138705408 B。⚠️ 但它**没有独立命名的「桌面版」产品**，官网四大场景里的「图形工作站」指的是电力调度台与变电站监控席。做出来的镜像对标的是那类席位环境，不是办公终端——若交付验证对象是办公软件，其意义会明显下降 |
...
- **Bianbu OS、RevyOS**：只在做 RISC-V 交付时有意义。两者都是芯片生态期项目，客户是开发者与量产下游而非装机终端用户；Bianbu 的硬件支持还写死在自家 MUSE 系列。
...
**一处尴尬值得记下来。** 按《安全可靠测评》桌面附表，进了党政采购准入的国产桌面只有银河麒麟、统信、方德三家（§2.1）。前两家是本项目的被试，而**第三家方德恰好是 ISO 拿不到的那家**——它的 Pro 版 V5.0 与银河麒麟桌面 V11 是同批（2025 年第 3 号公告）过评的两款桌面，交付场景与被试重叠，技术上最该做。也就是说：按采购目录该做的第三个，恰恰卡在材料可得性上，而不是技术上。上面推荐的三个候选中，凝思与麒麟信安都**不在**桌面附表里（凝思与麒麟信安入列的都是各自的服务器版）。若交付验证要对标「进了党政采购目录的桌面」，这一点必须先说清。

**方法而不只是三个镜像。** 名录里 21 个 OS 里，有 10 个是 deb 系或 deb 系旁证明确，三条路径对它们大体可套；真正的门槛不在技术，而在 ISO 授权与「厂商血统不公开」这两件事上——上面那张排除表里，③「ISO 拿不到」筛掉的三家全属前者。
...
| 3 | **凝思安全操作系统**：取 `linxos-v6.0.99-el20.03` ISO，套 §4.1／§4.2 的 deb 系路径 | 优先 |
| 4 | 先确认凝思镜像的对标对象是调度席位而非办公终端，并在档位定位里写明 | 优先，先于 3 |
...
99 条的标题由脚本抓自该页的 `<title>`；另 26 条的页面不返回可用 `<title>`（目录页、JSON API、二进制文件、部分老站与 SPA 站点），标题为我们的描述性标注，逐条以「标题为人工标注」标出。这类条目的**内容**照样可核对——URL 打开即是，只是标题那一栏不宜当作原文引用。
