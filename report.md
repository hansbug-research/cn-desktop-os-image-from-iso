...
| 统信 UOS | **V20**（归档号 1060） | **2.28** / 8.3 | **政采点名全是它**[^R126] | ✔ 官方 cdimage | 4253089792 | `UOS 20` | — |
...
**`403` 与 `404` 的差别可以用来探路径是否存在，但这个信号不通用。** 统信 `cdimage-download.chinauos.com` 对存在但禁列举的目录回 403、对不存在的回 404，靠这个逐层试出 V20 归档在 `1060/` 而不是发行注记里的 `1070`——两套编号不是一回事，此前按 `1070` 枚举一直落空就是因为这个。反例是麒麟 `iso.kylinos.cn`：它对任何路径一律回 403，包括随机构造的不存在路径。**用这个信号之前必须先拿一个肯定不存在的路径做阴性对照**，否则会把「全部 403」误读成「全部存在」。
