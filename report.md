...
### 2.7 四家被试各自还有哪些在用的大版本

前面几节按「厂商」选被试，这一节按「大版本」再切一层。做这件事的直接原因是：**厂商官网挂在最前面的那个版本，往往不是现场装得最多的那个**。2026 年度中央国家机关桌面操作系统框架协议入围的是银河麒麟桌面 V10 与统信桌面 V20，而两家最新一代（V11、V25）都已通过安可测评却没进这一轮[^R91]。

判据不用厂商的营销页，用两类可核对的信号：**软件源索引的最后更新时间**（apt 的 `dists/<suite>/Release` 的 `Date:`、rpm 的 `repomd.xml` 的 `revision`）与**安全公告的流量**。两者都能第三方复核，且能区分「还在卖」与「还在修」。

**银河麒麟桌面**（apt suite 实测，全部取自 `archive.kylinos.cn`）

| suite | 对应版本 | `Release` 的 `Date:` | 说明 |
|---|---|---|---|
| `4.0.2sp4` | V4 SP4 | 2025-03-12 | 官网下载页已不列，源仍在更新 |
| `10.0` | V10 | 2025-03-21 | 存量维护 |
| **`10.1`** | **V10 SP1** | **2026-04-11** | 全场最新，主力；本项目被试之一 |
| `11.0` | V11 | 2025-08-25 | 已 GA；本项目被试之一 |

V4 那条值得单说：它的源只有 4608 个包且**不含 `libc6`**，是增量补丁源而非完整基础源。所以「源还在更新」能证明厂商仍在给 V4 出包，但不足以从中构建镜像。从 `gcc 4:5.3.1-1kord1` 可判断它属 Ubuntu 16.04 那一代，**具体 glibc 版本未实测**——源里没有那个包。

**统信 UOS 桌面**：两条线都在售。V20（构建号 1070，当前 1070u5，2026-04-23）与 V25（2500，2026-04-15）。**1070u5 比 V25 的首版还晚八天**。政采点名的是 V20：北京市政采网 2025–2026 的中标公告里 V20 出现多次而 V25 零命中[^R126]。

**麒麟信安**：桌面 ISO 公开发布的只有两个，而且**新的那个架构反而更窄**。

| 桌面 ISO | 架构 | glibc | 源/公告状态 |
|---|---|---|---|
| `KylinSec-Desktop-3.4-4A-2206` | x86_64 **与 aarch64** | 2.28 | 源停在 2024-11-19，安全公告 2024-10-31 后归零 |
| `KylinSec-Desktop-6-release-250704` | **仅 x86_64** | 2.38 | 本项目被试之一；官网「版本下载」页未收录它 |

服务器线（自带 Kiran 桌面栈）反而覆盖更全：`3.4-5` / `3.4.5-update2`（glibc 2.28，x86_64 + aarch64）、`3.5.2`（2.34，+loongarch64）、`V6` / `V6 SP1`（2.38，+loongarch64），三条的安全公告都更新到 2026-08。招标文件里写的是「麒麟信安桌面操作系统 V3」这种**不带小版本号的商务标签**，现场装的是哪一版取决于集成商手上的镜像。

**凝思**：`V6.0.x` 里的第三段**就是大版本**，不是补丁号。六条独立依据：厂商自己签发的 CPE 是 `cpe:/o:linxos:linxos:6.0.80` 与 `:6.0.100` 并列[^R127]、官网八个彼此独立的产品页、逐版本单独的软著登记与定价、各自独立的 ISO 与文档树，以及最实质的一条——**ABI 完全不兼容**。

| 大版本 | 血统 | glibc / GCC | 含图形工作站 | CSAF 安全公告 |
|---|---|---|---|---|
| V6.0.42 | 私有包格式 | 2.5 / 4.1.2 | 是（KDE） | 18 条，2026-08 仍在补 |
| V6.0.60 | Debian 6 squeeze | 2.11 / 4.4.5 | 是（GNOME） | 18 条；2025-08 还发过 **i386** 包 |
| V6.0.80 | Debian 8 jessie | 2.19 / 4.9 | 是（MATE） | 17 条 |
| **V6.0.100** | Debian 10 buster | 2.28 / 8.3 | 是（MATE） | 19 条；本项目被试之一 |
| V6.0.99 欧拉版 | openEuler 20.03 | 2.28 / 7.3 | 否，仅服务器 | **472 条**，唯一被系统化打补丁 |
| V6-G2 / V8 | openEuler 24.03 | 2.38 / 12.3 | 否 | 0 条；ISO 在注册墙后（401） |

**「给很老的 ABI 持续打补丁」本身就是存量证据。** 凝思 2025 至 2026 年仍在给 glibc 2.5 的 V6.0.42 与 glibc 2.11 的 V6.0.60 发安全补丁，后者 2025-08 还发过 i386 包——没有跑不掉的存量，厂商不会为十几年前的 ABI 维护 32 位构建。

**横向看，ABI 断层的位置在四家高度一致。** 上一代整齐落在 glibc 2.28 附近（统信 V20 = 2.28、麒麟信安 3.4-x = 2.28、凝思 V6.0.99/V6.0.100 = 2.28、银河麒麟 V10 SP1 = 2.31），新一代整齐跳到 2.38 + GCC 12 + 内核 6.6（银河麒麟 V11、统信 V25、麒麟信安 V6、凝思 V8）。**一条 glibc 2.28 的验证线能覆盖四家的全部存量主力，2.38 的线只能覆盖新一代。** 本项目现有五个被试里 2.38 占三个、2.31 与 2.28 各一个——**偏向新一代**，这是覆盖面上的已知倾斜。

### 2.8 更老的国产桌面 OS：取材可得性实测

「已被厂商 deprecate」与「现场没人用了」是两件事。这一节只回答一个可验证的问题：**那些老系统的 ISO 今天还能不能拿到，拿到的是不是真货。**

核验方法两步，都不需要下载整个镜像：一是 `HEAD` 拿 `Content-Length` 与 `Last-Modified`；二是用 HTTP Range 读偏移 32768 处的 **ISO9660 主卷描述符**（2048 字节），从中读出 `CD001` 标识、系统标识与卷标。第二步能挡住「文件名对但内容是别的东西」这种情况。

| 系统 | ISO | 来源 | 实测 |
|---|---|---|---|
| 中标麒麟桌面 6.0 x86_64 | `NeoKylin-Linux-Desktop-6.0-x86_64-B045-20141201.iso` | 官方 `download.cs2c.com.cn` | **已 404**。此前记录为 HTTP 200 / 3482405376 B，本轮复测文件已撤；站点与目录 autoindex 均正常 |
| 中标麒麟桌面 6.0 **i386** | `NeoKylin-Linux-Desktop-6.0-i386-B045-20141201.iso` | **archive.org**（第三方归档）[^R128] | **可取**。3667734528 B，站方提供 md5 `e132275c…`、sha1 `deb2aa4c…`；Range 读出卷标原文 `NeoKylin Linux Desktop 6.0`、系统标识 `LINUX` |
| 银河麒麟 2.0（NeoKylin Server） | `KYLIN-2.0-14A.iso` / `-14B.iso` | archive.org[^R129] | 可取。664764416 B / 448593920 B，各带 md5 |
| 麒麟信安桌面 3.4-4A | `KylinSec-Desktop-3.4-4A-2206-201547-x86_64.iso` | 官方镜像站 | 200，4572839936 B；aarch64 版 4575578112 B |
| 凝思 V6.0.42 | `linxos-v6.0.42-20250925-x86_64.iso` | 官方 | 200，4840722432 B |
| deepin 15.11 | `deepin-15.11-amd64.iso` | 官方归档 `cdimage.deepin.com/releases-archive/` | 200，2459430912 B，2019-07-19；**同目录带官方 `MD5SUMS` 与 `SHA256SUMS`** |

**中标麒麟这一条最值得单独说。** 它的桌面 ISO 官方已撤，但**软件源还在线**：`download.cs2c.com.cn/neokylin/desktop/everything/7.1/x86_64/os/` 有完整 `repodata/`，**28830 个包**，索引更新于 2023-10-17；`updates/` 下挂着 4.0 到 7.1 共七条线，架构覆盖 x86_64、i386、ls_64（龙芯）、sw_64（申威）、aarch64；`everything/6.0/x86_64/oem/dell/` 说明当年有戴尔预装机。从源索引直接读出的 ABI 基线是 **glibc 2.20 / GCC 4.9.3 / 内核 4.4.13 / systemd 216 / Python 2.7.8**，release 后缀 `nd7`。这组数字对应 Fedora 20/21 那一代，**不是 RHEL 7**（RHEL 7 是 glibc 2.17 + GCC 4.8）——名录里「历史上被 DistroWatch 归为 Red Hat 系」这句话应据此收窄。

对本项目而言，中标麒麟是个特殊的候选：**它不需要 ISO 也能构建**。有完整 repodata，`rpmmedia` 路径（§4.4）可以直接对着在线源跑，而那条路径本来就是为「介质自带 repodata」写的。给一个已停售系统维护到 2023 年的软件源，与凝思给 glibc 2.5 打补丁属同一类存量信号。

**deepin 官方归档站是另一个正面例子**：`cdimage.deepin.com/releases-archive/` 列出 **68 个历史版本**，最早到 10.06（2010 年），且逐版本附官方校验值。这是本次调研里取材条件最好的一家。

**archive.org 的检索有效但命中稀疏。** 用 `mediatype:software` 限定后，「NeoKylin」只命中 1 条真品，「中标麒麟」4 条里 3 条是噪声（Ventoy 仓库快照、无关工具集）。它能补官方已撤的缺口，但不能指望覆盖面。

**未找到直链的：统信 V20。** 它是四家里唯一「政采点名、但取材路径没打通」的。`cdimage-download.chinauos.com` 的目录列举返回 403（这与 §2.1 记录的「目录索引 401/403 不等于文件不可达」一致——我们取 V25 用的那条完整路径至今仍返回 200），但按 V25 的命名规律枚举 V20 的多种路径组合均为 404。所以这一条如实记为**未找到**，不是不存在。
